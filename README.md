...
| **安全高度** | `--min-lift-height`（默认 **0.35 m**）硬钳下发给 `liftctrl` 的绝对高度 |
...
| `--min-lift-height` | `0.35` | 下发给 `liftctrl` 的最小高度 (m)，保证抓钩离地不低于该值 |
...
**抓钩离地安全下限**：下发给 `liftctrl` 的绝对高度会被硬钳到 `>= --min-lift-height`（默认 0.35m）。坐标校验层仍允许负坐标（零点由现场定义），执行器下限是物理硬保护。

> 若把目标 Z 设成低于该下限，抓钩会停在下限处、Z 轴无法判定到位而最终超时。要在输入层直接拒绝过低目标，可再配 `--workspace-z-min 0.35`。
...
| `ki_pos` | **0.4** | X/Y 位置环限幅积分增益 (1/s²)；用于顶开电机死区造成的稳态残余误差，0 = 纯 PD |
| `integral_band` / `integral_output_limit` | 0.08 / 0.10 | 积分仅在 \|误差\| 小于该窗口内累加 (m)；积分对速度指令的贡献上限 (m/s) |
| `ki_pos_z` | **0.0** | Z 位置环积分增益（PLC 模式）。Z 的 `liftctrl` 是绝对位置伺服、执行器已把速度指令积分成高度设定值，再加积分项会构成双重积分并在目标附近往复，故默认关闭 |
| `arrival_pos_tol` | **0.02** | 到达判定位置容差 (m)，**同时用作 PD 速度指令死区**——进入该窗口后 `v_cmd=0` |
| `arrival_vel_tol` | 0.005 | 到达判定速度容差 (m/s) |
| `arrival_vel_tol_z` / `velocity_deadband_z` | **0.02 / 0.025** | Z 的到达/零速容差 (m/s)，仅 PLC 模式生效。抓钩高度是 10Hz 粗分辨率读数、只能差分估速，用 X/Y 的容差会让 Z 永远锁不上轴 |
| `arrival_pos_tol_z` | **0.04** | Z 位置死区的**滞环退出阈值**兼到达位置容差 (m)，仅 PLC 模式生效。进入死区仍按 `arrival_pos_tol`(2 cm) 判定，但要重新给速度指令必须误差超过这个值，避免带滞后的粗分辨率读数在容差边界抖进抖出、把设定值推来推去 |
| `reverse_guard_tol` | **0.10** | 防反向抽动保护带 (m)；\|误差\| 小于此值时禁止给出远离目标的速度指令 |
| `arrival_capture_pos_tol` | **0.02** | 到位捕获位置窗口 (m) |
| `arrival_cmd_tol` | **0.018** | 到位捕获速度指令窗口 (m/s) |
...
### Z 轴在目标高度附近反复上下移动、收不住

**现象**：抓钩降/升到指定高度后不停住，在目标附近来回升降，PD 最终以超时结束。

**机理**：Z 的执行链路与 X/Y 不同——`liftctrl` 是绝对位置伺服，`PlcActuator` 把 PD 输出的 `vz` **积分**成高度设定值下发。于是位置误差每变一次号，都会被积分成一段真实的升降行程，被 PLC 忠实执行。让误差在目标附近反复变号的因素有三个：

1. **双重积分**。位置环本身已含上面那个积分环节，再叠加 `ki_pos` 就是双重积分，低频相位滞后接近 180°。
2. **反馈纯延迟**。抓钩高度经 S7 轮询 + PLC 扫描获得，连接抖动期间 `_LiftHeightSanitizer` 还会沿用上一次可信读数（最长 2 s）。延迟 + 双重积分 = 自激振荡。
3. **死区没有滞环 + 设定值可以越过目标跑远**。粗分辨率、带滞后的读数在 2 cm 容差边界上抖进抖出，每出去一次就给一个方向不定的速度脉冲；而单调钳位只挡住逼近那一侧，另一侧不受限——PD 往往在设定值到达目标前就进了死区、设定值停在目标外侧几厘米，此时反向脉冲能把它一路推回去。

此外，Z 速度只能由粗分辨率高度差分估计，其噪声地板高于 `arrival_vel_tol`/`velocity_deadband`，到位去抖计数被噪声反复清零，于是抓钩明明停在目标 ±1 cm 内也永远锁不上轴。

**当前默认已修复**，四条配套措施：

1. `ki_pos_z=0` — Z 用纯 PD，去掉双重积分（仿真模式的 Z 是速度伺服，仍用 `ki_pos`）。
2. 设定值一旦进入目标 ±`z_hold_band`(0.03 m) 就永久限死在带内 — 结构上不可能被推着往复跑远。
3. Z 位置死区带滞环（退出阈值 `arrival_pos_tol_z`=0.04 m）— 读数在容差边界抖动不再触发速度脉冲。
4. 目标附近只在高度读数**静止**时才允许再修设定值 — 不去追一个还在变化（带滞后）的误差。

复现实验（反馈延迟 0.5~2.0 s、并叠加 3 cm 反馈偏差）：修复前把高度设定值推离目标 4~15 cm 并反复换向、PD 超时；修复后 6~12 s 收敛，进入目标附近后幅度超过 2 mm 的往复段为 0。

**代价**：Z 的到位判定容差放宽到 `arrival_pos_tol_z`(0.04 m)。抓钩高度的可达精度本来就受读数分辨率和受载绳伸长限制，判定窗口比死区滞环还严只会得到"停在原地却永远不锁轴"。

**诊断**：看终端 `[PD] ... z_h=` 这一列的高度设定值——正常是单调走到目标后保持不动；若它在目标两侧来回跑，就是上述回路自激。
