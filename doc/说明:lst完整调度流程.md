...
| 参数 | 默认值 | 用途 |
|------|--------|------|
| `approach_safe_z` | 1.0 m | 接近取货点 XY 联动时的 Z 安全高度 |
| `lift_cargo_safe_z` | 1.2 m | 夹取后带货上升的安全高度 |
...
- **目标**：`(x1, y1, lift_cargo_safe_z)`（1.2 m）。
...
- **运动控制**：PD（X/Y 叠加限幅积分项 `ki_pos`）位置到速度伺服，10 Hz 控制循环。
- **XY 位置**：SLAM / 定位反馈，`big_car_ctrl`/`small_car_ctrl` 是速度伺服。
- **Z 位置**：优先使用 PLC `GetActualLiftHeight()` 抓钩实测高度；`liftctrl` 是**绝对位置伺服**，执行器把 PD 输出的 `vz` 积分成高度设定值，单调逼近目标，一旦进入目标 ±`z_hold_band`(0.03 m) 就永久限死在该带内。
- **Z 防目标附近上下往复**（现场故障"Z 收不住、在目标点附近切换方向"）。根因是"速度指令→高度设定值"这一步是积分环节，位置误差每变一次号就被积分成一段真实升降行程并被 PLC 忠实执行；四条配套措施：
  1. `ki_pos_z=0`：Z 用纯 PD，避免与上面那个积分环节构成双重积分（仿真的 Z 是速度伺服，仍用 `ki_pos`）。
  2. 设定值进入 ±`z_hold_band` 后限死在带内：反馈滞后/偏差再怎么让误差变号，也不可能被推着往复跑远。
  3. Z 位置死区带滞环（退出阈值 `arrival_pos_tol_z`=0.04 m）：粗分辨率、带滞后的读数在 2 cm 容差边界抖进抖出，不再触发方向不定的速度脉冲。
  4. 目标附近只在高度读数**静止**（`|v̂z| < arrival_vel_tol_z`）时才允许再修设定值：不去追一个还在变化的过时误差，最后一步交给 PLC 自己的位置环走完。
- **Z 到位容差单列**（`arrival_vel_tol_z`/`velocity_deadband_z`/`arrival_pos_tol_z`）：抓钩高度是 10 Hz 粗分辨率读数、只能差分估速，沿用 X/Y 的容差会让 Z 永远锁不上轴；位置容差必须与死区滞环退出阈值一致，否则会出现"抓钩停在原地却永远不锁轴"，只能等 PD 超时。
- **到位精度**：XY 残余误差约 0.02 m 量级；Z 判定容差 0.04 m，实测残余通常 0.01~0.03 m（受抓钩高度反馈分辨率与受载绳伸长限制）。
